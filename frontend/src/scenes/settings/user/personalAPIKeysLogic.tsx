...
export const personalAPIKeysLogic = kea<personalAPIKeysLogicType>([
    ...
    actions({
        ...
        showRotateKeySuccessDialog: (key: PersonalAPIKeyType, prevMaskedValue?: string | null) => ({
            key,
            prevMaskedValue,
        }),
        updateKey: (data: Partial<Pick<PersonalAPIKeyType, 'label' | 'scopes'>>) => data,
        deleteKey: (id: PersonalAPIKeyType['id']) => ({ id }),
        rotateKey: (id: PersonalAPIKeyType['id']) => ({ id }),
        ...
        keys: [
            [] as PersonalAPIKeyType[],
            {
                ...
                rotateKey: async ({ id }) => {
                    ...
                    const rotatedKey = await api.personalApiKeys.rotate(id)
                    actions.showRotateKeySuccessDialog(rotatedKey, origKey.mask_value)

                    // avoid persisting the raw value in state
                    rotatedKey.value = undefined
                    return values.keys.map((filteredKey) => (filteredKey.id == id ? rotatedKey : filteredKey))
                },
            },
        ],
    ...
    listeners(({ actions, values }) => ({
        ...
        showRotateKeySuccessDialog: async ({ key, prevMaskedValue }) => {
            const value = key.value
            ...
            LemonDialog.open({
                title: 'Personal API key rotated',
                width: 536,
                content: (
                    <>
                        <p className="mb-4">Your key "{key.label}" has been rotated:</p>
